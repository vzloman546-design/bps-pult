...
assert.match(sw,/const VERSION = '2\.1\.0'/);
for (const asset of ['styles.css','stability-logic.js','event-logic.js','knowledge-logic.js','app.js','event-ui.js','knowledge-ui.js']) {
  assert.equal(html.includes(`${asset}?v=2.1.0`),true,`${asset} должен иметь версионный URL`);
}
...
(async () => {
  ...
  const context={
    URL,
    caches:{open:async name=>{assert.equal(name,'bps-pult-2.1.0');return cache;},keys:async()=>[],delete:async()=>true},
